...
- Fixed bug where passing `params` to summary methods in `all_full()` and `all_full_concurrent()` was causing errors
- Summary operations now correctly exclude params intended for individual record fetching

## [0.2.1] - 2025-01-15

### Changed

- Moved `GetParams`, `UpdateParams`, and `CreateParams` to root module imports for easier access
- Updated documentation examples to use simplified `from netsuite_async import GetParams` syntax
